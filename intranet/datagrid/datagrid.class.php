...
## --------------------------------------------------------------------------- #
##  ApPHP DataGrid Pro (AJAX enabled) version 8.1.8                            #
##  Developed by:  ApPHP <info@apphp.com>                                      #
##  License:       GNU LGPL v.3                                                #
##  Site:          http://www.apphp.com/php-datagrid/                          #
##  Copyright:     ApPHP DataGrid (c) 2006-2016. All rights reserved.          #
##  Last changed:  20.01.2016 14:23                                            #
##                                                                             #
...
// IsOperationCompleted             SetSqlLimit                 dgGetDbDriverType 
// IgnoreBaseTag                    SetSqlLimitByDbType         dgGetDbDriverType 
// ExecuteSQL                       SetRealEscapeStringByDbType dgFetchRow    
// SelectSqlItem                    SetSqlByDbType				dgQuery
...
class DataGrid
{
    ...
    // Data Members
    //==========================================================================
    static private $dgVersion = '8.1.8';
    ...
    protected function DrawColumnar(){
        ...
        if($this->isLoadingImageEnabled && $this->methodPostBack != 'ajax') echo '<div id="'.$this->uniqueRandomPrefix.'loading_image" class="dg_loading_image">'.$this->lang['loading_data'].' <img src="'.$this->include_path.'images/loading.gif" alt="'.$this->lang['loading_data'].'" /></div>'.$this->nl;
        echo '<form style="margin:10px auto;padding:0px;" name="'.$this->uniquePrefix.'frmEditRow" id="'.$this->uniquePrefix.'frmEditRow" method="post" action="'.(($this->methodPostBack == 'get') ? $curr_url : '').'">'.$this->nl;
        echo '<input type="hidden" name="'.$this->uniquePrefix.'_operation_randomize_code" value="'.$this->GetRandomString(20).'" />'.$this->nl;
        $this->PrintHiddenFieldsFromUrl($curr_url);
        ...
        if($this->rid == -1){            
            foreach($this->columnsEditMode as $key => $val){
				...
				if (is_numeric($r)) { if($r % 2 == 0) $this->RowOpen($r, $this->rowColor[0]);
                	else $this->RowOpen($r, $this->rowColor[1]);}
                    ...
                    case 'label':
                        ...
                        if(($field_value_trim != '')
                            ...
                            if($fp_movable === true || $fp_movable == 'true'){
                                ...
                                $output .= '<img src="'.$this->directory.'images/up.png" style="margin-bottom:2px" border="0" alt="up" title="'.$this->lang['move_up'].'" />';
                                ...
                                $output .= '<img src="'.$this->directory.'images/down.png" style="margin-top:2px" border="0" alt="down" title="'.$this->lang['move_down'].'" />';
                                if(!$this->isPrinting) $output .= '</a>';
                            }                        
                        }else{
                            $output = $this->nbsp;
                            ...
                            if($this->GetFieldProperty($field_name, 'host') == 'remote'){                                
                                ...
                                if($file == true){
                                    ...
                                    if($field_type == 'image'){ 
                                        ...
                                        $ret_file .= $this->nbsp.'<img src="'.$this->directory.'images/mimetypes/'.Helper::GetFileMimetype($field_value).'.png" alt="'.Helper::GetFileMimetype($field_value).'" style="margin:0 0 -3px 5px;width:16px;height:16px;" />'.$this->nbsp;                                        
                                    }
                    ...
                    switch($field_type){
                                ...
                                if($fp_allow_downloading){
                                    if($blob_type != '' && $blob_name != '' && $blob_size != '' && strlen($field_value) != 0){
                                        ...
                                        if($post_type == 'session'){
                                            ...
                                            $output .= ' - [<img src="'.$this->directory.'images/download.png" alt="download" style="margin-bottom:-3px" border="0" /> <a class="'.$this->cssClass.'_dg_a" href="javascript:void(\'download\')" onclick="window.location.href=\''.$this->directory.'scripts/download_blob.php?fn='.$blob_name.'\';this.style.display=\'none\';" title="'.$this->lang['click_to_download'].'" download>'.$this->lang['download'].'</a>]';
                                        }else{
                                            $frid = isset($row[$this->primaryKey]) ? $row[$this->primaryKey] : '';
                                            $output .= ' - [<img src="'.$this->directory.'images/download.png" alt="download" style="margin-bottom:-3px" border="0" /> <a class="'.$this->cssClass.'_dg_a" href="javascript:void(\'download\')" onclick="window.location.href=\''.$this->directory.'scripts/download_blob.php?frid='.$frid.'\';this.style.display=\'none\';" title="'.$this->lang['click_to_download'].'" download>'.$this->lang['download'].'</a>]';
                                        }
                                    }
                                }
                            }else{
                                $output .= '<img src="'.$this->directory.'images/mimetypes/'.Helper::GetFileMimetype($field_value).'.png" alt="'.Helper::GetFileMimetype($field_value).'" style="margin:0 5px -3px 5px;width:16px;height:16px;" />';
                                if($fp_allow_downloading) $output .= '[<img src="'.$this->directory.'images/download.png" alt="download" style="margin-bottom:-3px" border="0" /> <a class="'.$this->cssClass.'_dg_a" href="'.$fp_target_path.$field_value.'" title="'.$this->lang['click_to_download'].'" download>'.$this->lang['download'].'</a>]';
                            }
                            $output = trim($output);                         
                            break;
                        case 'hidden':
                        ...
                        case 'image':
                            ...
                            if((trim($field_value) !== '') && $this->IsFileExists($fp_target_path.trim($field_value))){
                                ...
                                $ret_image = $ret_image_img;
                                if($fp_allow_downloading && !$this->isPrinting) $ret_image .= '<br />[<img src="'.$this->directory.'images/download.png" alt="download" style="margin-bottom:-3px" border="0" /> <a class="'.$this->cssClass.'_dg_a" href="'.$img_src.'" title="'.$this->lang['click_to_download'].'" download>'.$this->lang['download'].'</a>]';
                                $output = $ret_image;                            
                            }else{
    ...
    protected function PrepareTooltip($field_name = '', $mode = 'view'){
        ...
        if(!$this->isPrinting && $fp_header_tooltip != ''){
            ...
            $output = ' <img src="'.$this->directory.'styles/'.$this->cssClass.'/images/question_mark.gif" class="'.$this->cssClass.'_dg_question_mark" alt="question mark" '.$tooltip_title.' />';
        }
        return $output;        
    }

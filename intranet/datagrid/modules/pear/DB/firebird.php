 ...
 */
class DB_firebird extends DB_common
{
    // {{{ properties
     ...
     * @var string
     */
    var $phptype = 'firebird';

    /**
     * The database syntax variant to be used (db2, access, etc.), if any
     * @var string
     */
    var $dbsyntax = 'firebird';
    ...
    {
        if (!PEAR::loadExtension('pdo_firebird')) { // per la release 5 di php altrimenti usare interbase VDP
        ...
        $params = array(
            $dsn['hostspec'] ? ($dsn['hostspec'] . ':' . $dsn['database']) : $dsn['database'],
     ...
     */
     ...
     function numRows($result)
     {
        ...
        $result_local = ibase_query($this->last_query);
        while (ibase_fetch_row($result_local))
            $rows++;
        $this->freeResult($result_local);
        if (!$rows) {
            return $this->ibaseRaiseError();
        }
        return $rows;

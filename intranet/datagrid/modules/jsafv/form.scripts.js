...
function afv_isEmail(s){ if(afv_isEmpty(s)) if(afv_isEmail.arguments.length==1) return DEOK; else return(afv_isEmail.arguments[1]==true); regexp = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,7})$/; if(regexp.test(s)){ return true; }else{ return false; } }
...
function afv_isURL(url){ regexp = /^(([\w]+:)?\/\/)?(([\d\w]|%[a-fA-f\d]{2,2})+(:([\d\w]|%[a-fA-f\d]{2,2})+)?@)?([\d\w][-\d\w]{0,253}[\d\w]\.)+[\w]{2,7}(:[\d]+)?(\/([-+_~.\d\w]|%[a-fA-f\d]{2,2})*)*(\?(&?([-+_~.\d\w]|%[a-fA-f\d]{2,2})=?)*)?(#([-+_~.\d\w]|%[a-fA-f\d]{2,2})*)?$/; if(regexp.test(url)){ return true; } return false; }
